...
export const getBuildCommands = (params: BuildOptions) => {
  ...
  if (binaryVersion === 'latest') {
    ...
    return {
      ...
      dirOutputAndroid: ['all', 'android'].includes(platform) ? dirOutputAndroid : '',
...
function printLine(
  ...
  const outputLines = (data.toString() as string).split('\n').filter((line) => line.trim() !== '');
...
export function runCommand(name: string, command: string, taskOutputs: string[], maxLines = 25) {
...
export function checkBuiltVerions(taskOutputs: string[]) {
  ...
  if (builtVersions.versions.includes(AppConfig.version)) {
    taskOutputs.push(
      chalk.red.bgRed.bold('[ERROR]') + ' Version already built:' + AppConfig.version,
    );
    console.log(taskOutputs.join('\n'));
    process.exit(1);
  }
...
export function addBuiltVersion() {
  ...
  fs.writeFileSync(path.join('scripts', 'builtVersions.json'), JSON.stringify(builtVersions));
...
export function readBuiltVersions() {
  return JSON.parse(fs.readFileSync(path.join('scripts', 'builtVersions.json'), 'utf-8')) as {
    versions: string[];
  };
}
